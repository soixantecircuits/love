...
class UploadHandler
{
    ...
    $message->setSubject('Hey Soixante circuits - You\'ve got love');
    
    $loveimg = $message->embed(Swift_Image::fromPath("files/".$content[0]->name)); 
    
    $message->setBody(
      '<html>' .
      ' <head></head>' .
      ' <body style="height:100%;text-align:center;color:#BBB;display:block;background-color:#FFF;">' .
      '  <img style="position:relative;margin:0 auto;display:block;" src="' . $loveimg . '" alt="'.$content[0]->name.'"/>'.
      '   Salut'.
      '  </p>' .
      ' </body>' .
      '</html>',
    ...
    $message->addPart('You\'ve got some love', 'text/plain');

    
    $message->setReturnPath('contact@soixantecircuits.fr');
    $message->setFrom(array('contact@soixantecircuits.fr' => 'Some love is comming'));
    $message->addTo("love@soixantecircuits.fr");
    ...
    /*$transport = Swift_SmtpTransport::newInstance('mail.gandi.net', 25)
      ->setUsername('contact@soixantecircuits.fr')
    ->setPassword('xxx')
      ;*/
